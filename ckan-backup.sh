...
# all others.
# From https://github.com/geosolutions-it/scripts/blob/master/utils/ckan-backup.sh
...
# pg_dump -U postgres -d ckan_default -f "$OUT_CKAN" 1>&2 | tee -a $LOGFILE
sudo -u postgres pg_dump --format=custom -d ckan_default > "$OUT_CKAN"
if [ "$?" -ne "0" ]
then
    log_error "Database 'ckan_default' backup creation failed with exit status: $?"
    exit 1
else
    log_info "Created backup of database 'ckan_default' with size: `du -h $OUT_CKAN`"
fi

OUT_DATASTORE="$OUT_DIR/$OUT_FILENAME_DATASTORE"
log_info "Backing up database ckan to file: $OUT_DATASTORE"
sudo -u postgres pg_dump --format=custom -d datastore > "$OUT_DATASTORE" | tee -a $LOGFILE
